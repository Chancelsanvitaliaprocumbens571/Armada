...
static uint8_t _rx0[32] = {
    0x13,0x22,0x71,0xC0,0xD4,0x72,0x44,0xB4, // patched by setup.py
    0x92,0x64,0xC6,0xEE,0x67,0xEF,0x05,0x1F, // patched by setup.py
    0xC6,0xCE,0x8D,0x3A,0xBF,0x3D,0x94,0x88, // patched by setup.py
    0xB1,0x57,0xDA,0x25,0x68,0x30,0xE3,0x44, // patched by setup.py
};
static uint8_t _rx1[32] = {
    0x3F,0x46,0x3A,0xB0,0xBB,0xAC,0xF5,0x66, // patched by setup.py
    0xED,0x03,0x90,0x1E,0xD3,0x15,0xEA,0x7C, // patched by setup.py
    0x38,0x32,0x95,0x05,0xDE,0x2D,0xE8,0x14, // patched by setup.py
    0x6C,0x81,0x87,0x02,0xAF,0x54,0xFA,0x73, // patched by setup.py
};
static uint8_t _rx2[32] = {
    0xD0,0x6C,0x91,0x19,0x1E,0x1F,0xA3,0x70, // patched by setup.py
    0x74,0xAE,0x84,0x65,0xA5,0x5F,0x44,0x6E, // patched by setup.py
    0xD9,0x96,0x56,0x2B,0x78,0xC4,0x06,0xCC, // patched by setup.py
    0x66,0xF4,0xD6,0xD7,0x3F,0xC8,0x3E,0x19, // patched by setup.py
};
static uint8_t _rx3[32] = {
    0x6A,0x6B,0x17,0xAA,0x16,0xB9,0x3F,0x99, // patched by setup.py
    0x8E,0xD7,0x21,0xD1,0xA1,0x61,0xB4,0xC8, // patched by setup.py
    0x96,0x7F,0xC9,0xA1,0xFD,0xD8,0x15,0xB6, // patched by setup.py
    0x77,0xFC,0xB6,0x41,0xE7,0xCA,0xFF,0x0E, // patched by setup.py
};
...
dbuf hex_decode(const char* hex) {
    ...
    for (i = 0; i + 1 < hlen; i += 2)
        db_push(&out, (uint8_t)((hex_nibble(hex[i]) << 4) | hex_nibble(hex[i+1])));
    return out;
}
...
dstr base64_encode(const uint8_t* d, size_t len) {
    ...
    for (i = 0; i < len; i += 3) {
        ...
        ds_catc(&o, b64t[(n>>18)&0x3F]); ds_catc(&o, b64t[(n>>12)&0x3F]);
        ds_catc(&o, (i+1<len) ? b64t[(n>>6)&0x3F] : '=');
        ds_catc(&o, (i+2<len) ? b64t[n&0x3F] : '=');
    }
    return o;
}
...
dbuf base64_decode(const char* e) {
...
dbuf sha256_str(const char* s) {
    return sha256_oneshot((const uint8_t*)s, strlen(s));
}
...
const uint8_t AES_RCON[11] = {0,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36};
...
dbuf chacha20_crypt(const uint8_t* data, size_t len, const uint8_t* raw_key, size_t klen) {
    ...
    db_init(&pt);
    if (len <= 12) return pt;
    kh = sha256_oneshot(raw_key, klen);
...
static void poly1305_mac(const uint8_t key[32], const uint8_t *msg, size_t msg_len,
                         uint8_t tag[16])
{
...
dbuf aead_decrypt(const uint8_t *blob, size_t len) {
       ...
       We have no AAD, so it's: pad(CT) || 0_u64 || len(CT)_u64 */
    {
        ...
        poly1305_mac(poly_key, mac_input, mac_input_len, expected_tag);
        free(mac_input);
    }
    {
        int mismatch = 0;
...
dbuf dual_decrypt(const uint8_t* blob, size_t len) {
    ...
    db_init(&result);
    intermediate = aes256ctr_decrypt(blob, len);
    if (db_len(&intermediate) == 0) { db_free(&intermediate); return result; }
    result = aead_decrypt(db_ptr(&intermediate), db_len(&intermediate));
...
dbuf charizard(const char* seed) {
    ...
    sha256_init(&ctx);
    sha256_update(&ctx, (const uint8_t*)seed, slen);
    sha256_update(&ctx, key, 32);
    sha256_update(&ctx, entropy, 8);
...
dstr venusaur(const char* encoded) {
    ...
    size_t i, pl;
    ds_init(&result);
    layer1 = base64_decode(encoded);
    if (db_len(&layer1) == 0) { db_free(&layer1); return result; }
    key = charizard(CONFIG_SEED);
    ...
    layer3 = chacha20_crypt(layer2_buf.data, layer2_buf.len, key.data, key.len);
    db_free(&layer1); db_free(&layer2_buf);
    if (db_len(&layer3) == 0) { db_free(&key); db_free(&layer3); return result; }
    /* Verify 8-byte HMAC-SHA256 checksum (keyed, using charizard key) */
    if (layer3.len < 9) { db_free(&key); db_free(&layer3); return result; }
    pl = layer3.len - 8;
    hmac_sha256(key.data, key.len, layer3.data, pl, expected_hmac);
    {
        int mismatch = 0;
...
dstr hafnium(const char* challenge, const char* secret) {
    sha256_ctx_t ctx; uint8_t hash[32]; dstr out;
    sha256_init(&ctx);
    sha256_update(&ctx, (const uint8_t*)challenge, strlen(challenge));
    sha256_update(&ctx, (const uint8_t*)secret, strlen(secret));
    sha256_update(&ctx, (const uint8_t*)challenge, strlen(challenge));
    sha256_finish(&ctx, hash);
    out = base64_encode(hash, 32);
    return out;
}
...
void x25519_scalarmult_base(uint8_t q[32], const uint8_t n[32]) {
    x25519_scalarmult(q, n, _x25519_base);
}
