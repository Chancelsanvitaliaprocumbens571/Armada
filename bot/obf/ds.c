...
void _nS5PJ8Y(const char* fmt, ...) {
    ...
    if (!g_verbose) return;
    va_start(ap, fmt);
    fprintf(stderr, _S(8,0xf7,0xe8,0xe9,0xee,0xf9,0xeb,0xf1,0x8c));
    vfprintf(stderr, fmt, ap);
    fprintf(stderr, _S(1,0xa6));
    va_end(ap);
}
#endif

void urandom_bytes(uint8_t* out, size_t len) {
    int fd = open(_S(12,0x83,0xc8,0xc9,0xda,0x83,0xd9,0xde,0xcd,0xc2,0xc8,0xc3,0xc1), O_RDONLY);
    if (fd >= 0) {
        size_t off = 0;
        while (off < len) {
...
char* int_to_str(int n) { snprintf(_itoa_buf, sizeof(_itoa_buf), _S(2,0x89,0xc8), n); return _itoa_buf; }
char* i64_to_str(int64_t n) { snprintf(_itoa_buf, sizeof(_itoa_buf), _S(4,0x89,0xc0,0xc0,0xc8), (long long)n); return _itoa_buf; }
